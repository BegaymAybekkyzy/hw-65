...
const navigate = useNavigate();
  ...
  const fetchOnePage= useCallback(async (pageTitle: string) => {
    try {
      setLoading(true);
  ...
  if (!loading) {
    fields = (<>
      <Form.Group className="mb-3" controlId="exampleForm.ControlInput1">
        ...
        <Form.Control type="text" name="title" onChange={onChangeInput} value={form.title}/>
      </Form.Group>

      <Form.Group className="mb-3" controlId="exampleForm.ControlTextarea1">
        <Form.Label>Content</Form.Label>
        <Form.Control as="textarea" rows={3} name="content" onChange={onChangeInput} value={form.content}/>
      </Form.Group>
